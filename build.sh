...
docker build --build-arg GRAFANA_VERSION=${_grafana_version} --tag "nsymms/grafana-alpine:${_grafana_tag}" .

if [ $_release_build == true ]; then
	docker build --build-arg GRAFANA_VERSION=${_grafana_version} --tag "nsymms/grafana-alpine:latest" .
fi
